...
const CommentForm = ({ user, id, replyAt, getComments }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setErrMsg({ status: "failed", message: "An error occurred. Please try again." });
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className='w-full py-2' key={comment?._id}>
                ...
                <div className='ml-12'>
                  ...
                  {replyComments === comment?._id && (
                    <CommentForm
                      user={user}
                      id={comment?._id}
                      replyAt={comment?.userId?.firstName + " " + comment?.userId?.lastName}
                ...
                <div className='py-2 px-8 mt-6'>
                  {comment?.replies?.length > 0 && (
                    <p className='text-base text-ascent-1 cursor-pointer' onClick={() => setShowReply(showReply === comment?._id ? 0 : comment?._id)}>
